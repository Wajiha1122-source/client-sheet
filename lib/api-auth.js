...
export function formValues(formData, key) {
  return formData
    .getAll(key)
...
export async function requireApiUser(request, role) {
  ...
  try {
    ...
    const { rows } = await query(
      `SELECT users.id, users.name, users.username, users.role, users.job_title, users.office_id, offices.name AS office_name
       FROM users
