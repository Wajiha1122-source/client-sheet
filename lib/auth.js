...
export async function requireUser(role) {
  ...
  if (role && session.role !== role) redirect(homeForRole(session.role));

  const { rows } = await query(
    `SELECT users.id, users.name, users.username, users.role, users.job_title, users.office_id,
            offices.name AS office_name, offices.code AS office_code
     FROM users
